...
function barebones:ExperienceFilter(keys)
	...
	DebugPrint("Got experience from reason "..reason..", this much base exp: "..experience)
	if reason == DOTA_ModifyXP_CreepKill then
		keys.experience = math.floor(keys.experience * barebones:Get_POG_CREEP_EXPERIENCE_MULTIPLIER(playerID))
	end
...
function barebones:BountyRuneFilter(keys)
	...
	keys.gold_bounty = barebones:Get_POG_BOUNTY_RUNE_GOLD_AMOUNT(playerID)
