...
function barebones:InitGameMode()
	...
	LinkLuaModifier("modifier_pog_gg", "modifiers/modifier_pog_gg", LUA_MODIFIER_MOTION_NONE)
...
function barebones:CaptureGameMode()
	...
	if FORCE_PICKED_HERO ~= nil then
		...
		if ENABLE_BANNING_PHASE then
			...
			GameRules:SetCustomGameBansPerTeam(20)
		end
	end
...
function barebones:startRunes(rune_name)
	local first = true
	Timers:CreateTimer(function()
		local rune_spawn = Entities:FindByName(nil, rune_name):GetAbsOrigin()
		if first then
			CreateRune(rune_spawn, DOTA_RUNE_BOUNTY)
			first = false
		else
...
function barebones:shuffle(tbl)
	for i = #tbl, 2, -1 do
